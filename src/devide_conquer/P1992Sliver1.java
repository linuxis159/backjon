...
public class P1992Sliver1 {
    ...
    private static void recursive(int N,int xStart,int yStart){
        char compareValue = array[yStart].charAt(xStart);
        if(N != 1) {
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < N; j++) {
                        if (compareValue != array[yStart+i].charAt(xStart + j)) {
                            result += "(";
                            recursive(N / 2, xStart, yStart);
                            recursive(N / 2, (xStart + (xStart + N)) / 2, yStart);
                            recursive(N / 2, xStart, (yStart + (yStart + N)) / 2);
                            recursive(N / 2, (xStart + (xStart + N)) / 2, (yStart + (yStart + N)) / 2);
                            result += ")";
                            return;
                        }
                }
