...
n장의 카드에 쓰여있는 수를 모두 더한 값이 이 놀이의 점수가 된다. 이 점수를 가장 작게 만드는 것이 놀이의 목표이다.
...
public class P15903_카드합체놀이 {
    static Scanner sc = new Scanner(System.in);
    static long cardNum[];
    public static void main(String[] args){
        int cardCnt = sc.nextInt();
        int mergeCnt = sc.nextInt();
        cardNum = new long[cardCnt];
        for(int i=0; i<cardCnt; i++){
            cardNum[i] = sc.nextInt();
        }
        for(int i=0; i<mergeCnt; i++){
            Arrays.sort(cardNum);
            long merged = cardNum[0] + cardNum[1];
            cardNum[0] = merged;
            cardNum[1] = merged;
        }
        long result = 0;
